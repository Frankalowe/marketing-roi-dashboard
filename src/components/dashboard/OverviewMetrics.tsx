    ...
    return (
        <div className="space-y-3 animate-fade-in-up h-full flex flex-col">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-2">
                {cards.map((card) => (
                    <Card key={card.name} className="p-3 group relative hover:shadow-md transition-all duration-300 border-slate-200/60 flex flex-col justify-between">
                        ...
                        <div>
                            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-[0.15em] mb-0.5 truncate">{card.name}</p>
                            <p className="text-base font-bold text-slate-900 tracking-tight tabular-nums">{card.value}</p>
                        </div>
                    </Card>
                ))}
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-3 flex-1 min-h-0">
                <Card className="lg:col-span-4 p-0 h-full flex flex-col">
                    <CardHeader
                        ...
                        className="py-3 px-4"
                    />
                    <CardContent className="space-y-3 pt-0 px-4 pb-4 flex-1 flex flex-col justify-center">
                        <div className="space-y-3">
                            <div className="group">
                                <div className="flex justify-between items-end mb-2">
                                    ...
                                    <span className="text-lg font-bold text-slate-900 tabular-nums">{stats.localCalls}</span>
                                </div>
                            ...
                            <div className="group">
                                <div className="flex justify-between items-end mb-2">
                                    ...
                                    <span className="text-lg font-bold text-slate-900 tabular-nums">{stats.whatsappCalls}</span>
                                </div>
                                <div className="h-1 w-full bg-slate-50 border border-slate-100/50 rounded-full overflow-hidden">
                                    <div
                        ...
                        <div className="pt-3 border-t border-slate-50 mt-auto">
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <p className="text-lg font-bold text-slate-900 tracking-tight">{conversionRate.toFixed(2)}% <span className="text-[9px] font-bold text-emerald-600 bg-emerald-50 border border-emerald-100 px-2 py-0.5 rounded ml-2 uppercase tracking-wide">Acquisition</span></p>
                                </div>
                                <div className="p-2 rounded-xl bg-blue-50 text-blue-600 shadow-sm ring-1 ring-blue-500/10">
                ...
                <Card className="lg:col-span-8 p-0 overflow-hidden flex flex-col h-full">
                    <CardHeader
                    ...
                    <CardContent className="flex-1 min-h-[200px] p-0 relative bg-slate-50/30 overflow-hidden">
                        {/* Map Visualization */}
